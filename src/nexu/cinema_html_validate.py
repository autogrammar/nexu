...
def validate_cinema_html_document(html: str, *, ui_type: str = "web") -> tuple[bool, list[str]]:
    ...
    # Imported pages may contain original WordPress/theme CSS that would be
    # unsafe if generated by an LLM, but is required to render the captured page.
    # Local repatch changes are validated through their scoped CSS helpers.
    if ui_type not in {"imported", "http", "wordpress"}:
